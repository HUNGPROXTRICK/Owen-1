...
module.exports.run = async function({ api, event, args, Users, Threads }) {
  ...
  if (query == 'box') {
    ...
    return require('./box.js').run(...Object.values(arguments));
      ...
      let ndfb = thread_info.userInfo.filter($=>$.type == 'UnavailableMessagingActor');
...
send(`Có ${ndfb.length} người dùng Facebook, reaction để xóa TV`, (err, res)=>(res.name = exports.config.name, res.event = o.event, res.ndfb = ndfb, global.client.handleReaction.push(res)));
    }
  else if (query == 'reset') {
     let dataThread = (await Threads.getData(threadID)).threadInfo;
    if (!dataThread.adminIDs.some(item => item.id == senderID)) return api.sendMessage('Bạn không đủ quyền hạn để sử dụng!', event.threadID, event.messageID);
...
if ((!check && Object.keys(mentions).length == 0) || (!check && Object.keys(mentions).length == 1) || (!check && event.type == 'message_reply')) {
    ...
    body += `[ 𝐎𝐖𝐄𝐍 ]
        ...
        🌾 Tỉ Lệ Tương Tác: ${isFinite(interaction_rate_day)?interaction_rate_day.toFixed(1)+'%' :'chưa có tin nhắn hôm qua nên không thể tính tỉ lệ'}
        🥇 Hạng Trong Ngày: ${userRankDay + 1}
        ───────────────
        💬 Tin Nhắn Trong Tuần: ${userTotalWeek}
        🌾 Tỉ Lệ Tương tác: ${isFinite(interaction_rate_week)?interaction_rate_week.toFixed(1)+'%': 'chưa có tin nhắn tuần qua nên không thể tính tỉ lệ'}
